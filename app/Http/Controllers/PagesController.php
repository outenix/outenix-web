...
use App\Http\Controllers\TopTelegramGroupController;
use App\Http\Controllers\TopTelegramMessageController;
use App\Http\Controllers\DetailTelegramGroupController;
...
class PagesController extends Controller
{
    public function dashboard()
    {
        ...
        $wallet = $WalletsController->wallets();

        $TopTelegramGroupController = new TopTelegramGroupController();
        $topGroups = $TopTelegramGroupController->topTelegramGroups();

        $TopTelegramMessageController = new TopTelegramMessageController();
        $topMessages = $TopTelegramMessageController->topTelegramMessages();

        return view('publics.dashboard', compact('wallet', 'topGroups', 'topMessages'));
    }

    public function paginationTopUsers($group_id, $page, $sort, $search)
    {
        $DetailTelegramGroupController = new DetailTelegramGroupController();
        $topUsers = $DetailTelegramGroupController->getTopUsers($group_id, $page, $sort, $search);

        return view('partials.group-details.pagination-top-users', compact('topUsers'));
    }

    public function groupDetailProfile()
    {
        return view('partials.group-details.profile');
    }

    public function groupDetailTopUsers($group_id, $page, $sort, $search)
    {
        $DetailTelegramGroupController = new DetailTelegramGroupController();
        $topUsers = $DetailTelegramGroupController->getTopUsers($group_id, $page, $sort, $search);
        $groupDetails = $DetailTelegramGroupController->getGroupDetails($group_id);

        return view('partials.group-details.top-users', compact('topUsers','groupDetails'));
    }

    public function groupDetailStatistic()
    {
        return view('partials.group-details.statistic');
    }

    public function groupDetailHistory()
    {
        return view('partials.group-details.history');
    }

    public function settings()
    {
        return view('publics.settings');
    }
    ...
    public function groups($group_id)
    {
        ...
        return view('publics.apps-group', compact('groupDetails'));
    }    

    public function avatars()
    {
        $AvatarsController = new AvatarsController(); 
        $avatars = $AvatarsController->avatars();
        return view('publics.avatars', compact('avatars'));
    }
    ...
    public function settingAccount(Request $request)
    {
        ...
        if (!$request->ajax()) {
            // Jika bukan permintaan AJAX, arahkan ke halaman settings dengan parameter redirect
            $redirect = redirect('/settings')->with('redirect', 'settingAccount');
            
            // Cek jika session 'error' ada, tambahkan ke redirect dan log
            if (session('error')) {
                Log::error('Error Message: ' . session('error'));  // Menambahkan log untuk pesan error
                $redirect->with('error', session('error'));
            }
            
            // Cek jika session 'warning' ada, tambahkan ke redirect dan log
            if (session('warning')) {
                Log::warning('Warning Message: ' . session('warning'));  // Menambahkan log untuk pesan warning
                $redirect->with('warning', session('warning'));
            }
            
            // Cek jika session 'success' ada, tambahkan ke redirect dan log
            if (session('success')) {
                Log::info('Success Message: ' . session('success'));  // Menambahkan log untuk pesan success
        ...
        $settingaccount = $AccountsController->account();
    
        $AvatarsController = new AvatarsController();
        $settingavatar = $AvatarsController->avatar();
    
        return view('partials.settings.settingAccount', compact('settingaccount', 'settingavatar'));
    }
    
    public function settingSecurity(Request $request)
    {
        // Periksa apakah permintaan berasal dari AJAX
        if (!$request->ajax()) {
            // Jika bukan permintaan AJAX, arahkan ke halaman settings dengan parameter redirect
            return redirect('/settings')->with('redirect', 'settingSecurity');
        }
        
        return view('partials.settings.settingSecurity');
    }    

    public function settingDevice(Request $request, DevicesController $devicesController)
    {
        // Periksa apakah permintaan berasal dari AJAX
        if (!$request->ajax()) {
            // Jika bukan permintaan AJAX, arahkan ke halaman settings dengan parameter redirect
            return redirect('/settings')->with('redirect', 'settingDevice');
        }
    
        // Memanggil fungsi devices untuk mendapatkan daftar perangkat
        $data = $devicesController->devices($request);
    
        // Kembalikan view untuk pengaturan perangkat
        return view('partials.settings.settingDevice', [
    ...
    public function settingTheme(Request $request)
    {
        ...
        if (!$request->ajax()) {
            // Jika bukan permintaan AJAX, arahkan ke halaman settings dengan parameter redirect
            return redirect('/settings')->with('redirect', 'settingTheme');
        }

        return view('partials.settings.settingTheme');
    }    

    public function appLogs()
    {
         // Panggil fungsi errorQueryLogs dari ErrorQueryLogsController
         return app(LogsController::class)->errorQueryLogs();
    }
