...
Route::middleware(['guest'])->group(function () {
    Route::get('/login', [\App\Http\Controllers\AuthsMailController::class, 'loginView'])->name('loginView');
    Route::post('/login', [\App\Http\Controllers\AuthsMailController::class, 'login'])->name('login');
    Route::get('/register', [\App\Http\Controllers\AuthsMailController::class, 'registerView'])->name('registerView');
    Route::post('/register', [\App\Http\Controllers\AuthsMailController::class, 'register'])->name('register');

    Route::get('auth/google', [\App\Http\Controllers\AuthsGoogleController::class, 'redirectToGoogle'])->name('auth.google');
    Route::get('auth/google/callback', [\App\Http\Controllers\AuthsGoogleController::class, 'loginGoogle'])->name('auth.google.callback');

    Route::post('/auth/set-cookie', [\App\Http\Controllers\AuthsGoogleController::class, 'setAuthCookie'])->name('auth.setCookie');
    Route::get('/auth/set-cookie', function () {
...
Route::middleware('auth', 'check.status')->group(function () {
    ...
    Route::get('/avatars', [PagesController::class, 'avatars'])->name('avatars');   
    Route::get('/group/{group_id}', [PagesController::class, 'groups'])->name('group');

    // Route Partials Group Details
    Route::get('/group-details/history/{group_id}', [PagesController::class, 'groupDetailHistory'])->name('group-details/history');
    Route::get('/group-details/profile/{group_id}', [PagesController::class, 'groupDetailProfile'])->name('group-details/profile');
    Route::get('/group-details/statistic/{group_id}', [PagesController::class, 'groupDetailStatistic'])->name('group-details/statistic');
    Route::get('/group-details/top-users/{group_id}/{page}/{sort}/{search}', [PagesController::class, 'groupDetailTopUsers'])->name('group-details/top-users');
    Route::get('/group-details/p/top-users/{group_id}/{page}/{sort}/{search}', [PagesController::class, 'paginationTopUsers'])->name('group-details/p/top-users');

    // Route Partials Settings
    Route::get('/settings/account', [PagesController::class, 'settingAccount'])->name('settings/account');
    Route::get('/settings/security', [PagesController::class, 'settingSecurity'])->name('settings/security');
    Route::get('/settings/device', [PagesController::class, 'settingDevice'])->name('settings/device');
    Route::get('/settings/theme', [PagesController::class, 'settingTheme'])->name('settings/theme');

    // Route Functions
    Route::post('settings/redirect/google', [\App\Http\Controllers\ConnectAccountsController::class, 'redirectToGoogle'])->name('settings/redirect/google')->methods(['POST']);
    Route::get('settings/redirect/google', function () {
        return redirect()->route('dashboard');
    });
    Route::get('settings/redirect/google/callback', [\App\Http\Controllers\ConnectAccountsController::class, 'connectGoogle'])->name('settings/redirect/google/callback');

    Route::post('settings/device/delete', [\App\Http\Controllers\DevicesController::class, 'deleteDevice'])->name('settings/device/delete')->methods(['POST']);
    Route::get('settings/device/delete', function () {
        return redirect()->route('dashboard');
    });

    Route::post('settings/device/clear', [\App\Http\Controllers\DevicesController::class, 'clearDevices'])->name('settings/device/clear')->methods(['POST']);
    Route::get('settings/device/clear', function () {
        return redirect()->route('dashboard');
    }); 

    Route::post('settings/account/edit', [\App\Http\Controllers\AccountsController::class, 'editAccount'])->name('settings/account/edit')->methods(['POST']);
    Route::get('settings/account/edit', function () {
        return redirect()->route('dashboard');
    });

    Route::post('/logout', [\App\Http\Controllers\AuthsLogoutController::class, 'logout'])->name('logout')->methods(['POST']);
    Route::get('/logout', function () {
        return redirect()->route('dashboard');
    });

    Route::get('/app/logs', [PagesController::class, 'appLogs'])->name('app/logs');
    Route::get('/refresh-error-query-logs', [LogsController::class, 'refreshErrorQueryLogs'])->name('refresh-error-query-logs');
    Route::get('/search-error-query-logs', [LogsController::class, 'searchErrorQueryLogs'])->name('search-error-query-logs');

    Route::get('/error/404', [PagesController::class, 'layoutsError404'])->name('/error/404');
